...
exports.imagePostSize =async (req,res,next)=>{
    ...
    await sharp(req.file.path).resize(500,500).toFormat('jpeg').jpeg({quality: 98}).toFile(`public/images/post/${req.filename}`)
    fs.unlinkSync(`public/images/post/${req.filename}`)
    next()
}